...
async function fetchAnnoucment(): Promise<Announcement | null> {
	try {
		const response = await axios.get(`${process.env.SITE_BASE_URI}/api/get_announcement`);

		if (response.data.success) {
			return response.data.announcement;
		}
		console.log(`API ${process.env.SITE_BASE_URI}/api/get_announcement returned success: false`)
		return null
	}
	catch (error: any) {
