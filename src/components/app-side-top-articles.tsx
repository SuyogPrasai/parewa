...
const SideArticleCard = ({ article, rank }: { article: Article; rank: number }) => {
    ...
    return (
        <Card className="h-[75px] sm:h-[85px] overflow-hidden rounded-lg flex w-full max-w-full lgplus:max-w-[290px] mx-auto border border-gray-200 p-2 py-3 items-center transition-all duration-300 ease-in-out hover:shadow-md">
            <Link href={article.link} className="flex flex-row justify-start w-full overflow-hidden">
                ...
                <div className="flex flex-col overflow-hidden w-full gap-1">
                    <div className="flex items-center gap-2">
                        <h3 className="text-xs sm:text-sm md:text-[14px] lg:text-[15px] font-bold font-sans truncate whitespace-nowrap overflow-hidden w-full hover:text-blue-600 transition-colors duration-300">
                            {article.title}
                        </h3>
                    </div>
